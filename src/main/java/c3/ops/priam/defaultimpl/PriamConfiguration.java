...
public class PriamConfiguration implements IConfiguration {
   ...
   private static final String CONFIG_US_EAST_2_S3_ENDPOINT  = PRIAM_PRE + ".useast2.s3url";
   private static final String CONFIG_AP_SOUTHEAST_2_S3_ENDPOINT  = PRIAM_PRE + ".apsoutheast.s3url";
   private static final String CONFIG_SA_EAST_1_S3_ENDPOINT  = PRIAM_PRE + ".saeast1.s3url";
   // Amazon specific
   private static final String CONFIG_Ring_Name              = PRIAM_PRE + ".az.ringname";
   ...
   private static final String DEFAULT_US_EAST_2_S3_ENDPOINT = "s3-us-east-2.amazonaws.com";
   ...
   private static       String US_EAST_2_REGION              = "us-east-2";
   ...
   private final  String RAC            =
         SystemUtils.getDataFromUrl("http://169.254.169.254/latest/meta-data/placement/availability-zone");
   private final  String LOCAL_HOSTNAME =
         SystemUtils.getDataFromUrl("http://169.254.169.254/latest/meta-data/local-hostname").trim();

   private final String LOCAL_IP      =
         SystemUtils.getDataFromUrl("http://169.254.169.254/latest/meta-data/local-ipv4").trim();
   private final String INSTANCE_ID   =
         SystemUtils.getDataFromUrl("http://169.254.169.254/latest/meta-data/instance-id").trim();
   private final String INSTANCE_TYPE =
         SystemUtils.getDataFromUrl("http://169.254.169.254/latest/meta-data/instance-type").trim();
   ...
   private final String DEFAULT_SEED_PROVIDER         = "c3.ops.cassandra.cassandra.extensions.NFSeedProvider";
   ...
   private void setupEnvVars() {
      ...
      Ring_Name = StringUtils.isBlank(Ring_Name) ? System.getProperty("Ring_Name") : Ring_Name;
   ...
   private void populateProps() {
      config.set(CONFIG_Ring_Name, Ring_Name);
   ...
   public String getRingName() {
      return config.get(CONFIG_Ring_Name, "");
   }
   ...
   public String getS3EndPoint() {
      ...
      if (US_EAST_2_REGION.equals(region)) {
         s3Url = config.get(CONFIG_US_EAST_2_S3_ENDPOINT);
         return StringUtils.isBlank(s3Url) ? DEFAULT_US_EAST_2_S3_ENDPOINT : s3Url;
      }
   ...
   public String getDseClusterType() {
      return config.get(CONFIG_DSE_CLUSTER_TYPE + "." + Ring_Name, "cassandra");
   }
